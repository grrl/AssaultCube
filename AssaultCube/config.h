...
enum confParams //modi f・ configfiles
{
...
public:
	...
	cConfig(std::string configpath, confParams configMode); // konstruktor mit fnen der streams
	~cConfig(); // destruktor der die streams schliesst


	std::string getPath(); //wrapper f・ strConfigPath


	template<class write_T> // template f・ Write um in die config zu schreiben
	inline bool Write(std::string strOptName, write_T tValue); // funktion um in datei zu schreiben

	template<typename read_T> // teamplate f・ read um aus der config zu lesen
	inline read_T Read(std::string strOptName);

	template<class read_T> // teamplate f・ read um aus der config zu lesen
...
private:
	...
	std::string	strConfigPath = ""; // Pfad f・ Config
	std::ofstream outputFileStream; //outputfilestream der ・erall verwendet werden soll also sollte. z・d mich an wenns nicht so ist.
	std::ifstream inputFileStream; //inputFileStream der ・erall verwendet werden soll also sollte. z・d mich an wenns nicht so ist.

};


template<class write_T> //template damit man nicht tausende ・erladungen hat
inline bool cConfig::Write(std::string strOptName, write_T tValue) // schreibt einen wert in eine datei
{
	...
	if (!this->outputFileStream.good()) //gibt false zur・k wenn etwas mit der datei nicht stimmt.
		return false;

	this->outputFileStream << strOptName << " = \"" << tValue << "\";\n"; // schreibt in die datei

	return true; // gibt true zur・k wenn alles glattlief
...
std::string FORCEINLINE strGetValueFromLine(std::string strLine) //gibt den string der value der line z・uck
{
	return splitstring<std::string>(splitstring<std::string>(strLine, " = \"").at(1), "\";").at(0);
}
std::string FORCEINLINE strGetOptionNameFromLine(std::string strLine) //gibt den string des optionsnamen von der line zur・k
{
...
inline read_T cConfig::Read(std::string strOptName)
{
	...
	this->openInputStream(); //filestream dann wieder 臟fnen um lesen zu knen
